"""
Producteur Kafka — PORTÉ DE services/peer/app/kafka/producer.py, sans modification
fonctionnelle. Mêmes topics, même sémantique (doc 00 §7) : la structure des topics Kafka
n'a pas besoin de changer pour la refonte CIVITAS Agent, seul le point d'exécution qui les
publie change (un process par room au lieu d'un process pour toutes les rooms).
"""
import asyncio
import json
import logging
...
async def _reconnect_loop():
    """
    Retente indéfiniment en arrière-plan, avec backoff, tant que non connecté — cf.
    justification identique dans l'ancien peer : sans ça, une indisponibilité Kafka au
    démarrage tuerait silencieusement toute publication pour toute la durée de vie du process
    (ici : pour la durée de vie de l'agent d'UNE SEULE room, ce qui limite déjà nativement le
    rayon d'impact par rapport à l'ancien peer partagé).
    """
    backoff = 5
...
async def publish_transcription(
    ...
    payload = {
        ...
        "text": text, "entry_type": entry_type, "source": "civitas-agent",
    }
    if extra:
        payload["extra"] = extra
...
async def publish_agent_action(room_id: str, action: str, details: dict = {}):
    await publish("room.agent.actions", room_id, {
        ...
        "details": details, "source": "civitas-agent",
    })
...
async def publish_room_event(room_id: str, event_type: str, data: dict = {}):
    await publish("jitsi.room.events", room_id, {
        ...
        "data": data, "source": "civitas-agent",
    })
