"""
Handlers d'événements Jitsi — un handler par domaine.

Architecture :
  Chaque handler est une fonction async indépendante.
  On les compose via EventBus.register().
  Facilement extensible pour LangGraph (ajouter un handler = une ligne).

Handlers disponibles :
  - make_speaker_handler    : met à jour SpeakerTracker
  - make_kafka_handler      : publie sur Kafka
  - make_log_handler        : log structuré
  - make_chat_handler       : déclenche la réponse CIVITAS
  - make_moderation_handler : actions de modération (levé de main, etc.)
"""
import logging
...
def make_speaker_handler(tracker) -> Callable:
    """
    Met à jour le SpeakerTracker à chaque événement participant/audio.
    Doit être le PREMIER handler enregistré (les autres en dépendent).
    """
...
def make_log_handler(room_id: str) -> Callable:
    ...
    async def handler(event_type: str, data: dict):
        ...
        icons = {
            ...
            "USER_LEFT":                 "👤-",
            "USER_ROLE_CHANGED":         "🔑",
            "DISPLAY_NAME_CHANGED":      "✏️",
            "DOMINANT_SPEAKER_CHANGED":  "🎙️",
            "TALK_WHILE_MUTED":          "🔇",
            "NOISY_MIC":                 "📢",
            "TRACK_MUTE_CHANGED":        "🔊",
            "MESSAGE_RECEIVED":          "💬",
            ...
            "REACTION_RECEIVED":         "😊",
            "PARTICIPANT_PROPERTY_CHANGED": "⚙️",
            "POLL_RECEIVED":             "📊",
...
def make_kafka_handler(room_id: str, kafka) -> Callable:
    """Publie les événements pertinents sur Kafka."""
    async def handler(event_type: str, data: dict):
        if event_type in ("AUDIO_LEVEL", "DOMINANT_SPEAKER_CHANGED"):
            # DOMINANT_SPEAKER publié avec le snapshot complet dans instance.py
            return
...
def make_moderation_handler(room_id: str, browser, context: dict) -> Callable:
    """Réactions de modération automatique (levé de main, parle muté, etc.)."""
    async def handler(event_type: str, data: dict):
